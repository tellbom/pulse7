...
rem Guard: Start.exe calls can HANG when the Sandboxie service is dead, so
rem only touch them while SbieSvc is actually RUNNING.
sc query SbieSvc 2>nul | findstr RUNNING >nul
if errorlevel 1 (
  echo Sandboxie service not running - skipping box cleanup
  goto skipbox
)
if exist "%START%" (
...
rem remove product dir (delayed + retried so this script and any late
rem file-handle releases finish first
cd /d "%TEMP%"
start /b cmd /c "for /l %%i in (1,1,6) do ( ping -n 3 127.0.0.1 >nul & rmdir /s /q "%B%" & if not exist "%B%" exit )"
