# pulse7 项目 — 完整交付文档（v2，更新至 RC 0.3.3）

> 2026-09-10 G5 用户配置补充：write/edit 的 allow、deny 规则按解析后的最终目标匹配。请填写最终绝对路径，junction 别名不会同时参与匹配。例如执行 `mklink /J C:\work\shared D:\shared` 后，经 `C:\work\shared\file.txt` 写入实际命中 `D:\shared\file.txt`：`{"tool":"write","pattern":"D:/shared/*","action":"deny"}` 会拒绝；把 pattern 写成 `C:/work/shared/*` 则不命中该次写入。edit 同理，需配置对应 tool。open 档位下未命中 deny 的写入会放行，工作区外写入仍有事件、审计和结束摘要。此补充描述现状，不改变匹配行为；不将 write/edit 的规则外推到其他工具。

> 合并项目从立项到 RC 0.3.3 的全部关键决策、架构、验证数据、差距分析与已知问题。供接手方（或 AI 助手）快速获得完整上下文。
...
| # | 问题 | 影响 | 状态 |
|---|---|---|---|
| 1 | R2 在 Sandboxie 模式下 1/3 收敛率，JobObject 下 3/3 | 探索类任务在默认模式下不可靠 | 记录（根因未明，留给试用反馈） |
| 2 | agent checkpoint 产物目录出现在工作区（.pulse7/）| git status 显示 untracked | 未修 |
| 3 | ripgrep 14+ 需 Win10 | 必须锁 13.0.0 | 已记录 |
| 4 | deepseek-v4-flash 返回 reasoning_content | token 含推理部分 | 兼容 |
| 5 | tree 仅在 5829 文件项目验证 | 18K+/50K+ 未验证 | 真实使用观察 |
| ~~6~~ | ~~edit CRLF/LF 换行符差异~~ | ~~S2 re-edit 循环~~ | **已修复**（RC 0.3.3：20→5 轮） |
| ~~7~~ | ~~afterShellCleanup /terminate 间歇性失败~~ | ~~S1/S2 不收敛~~ | **已修复**（RC 0.3.3：14%→100%） |
| ~~8~~ | ~~T4 重命名路径 bug（3 文件 6 处）~~ | ~~Sandboxie 全挂~~ | **已修复**（RC 0.3.1+0.3.2） |
...
| 制品 | 版本 | SHA256 |
|---|---|---|
| **pulse7-rc0.3.1.7z**（推荐） | RC 0.3.3 | `531d6ccd0d811fc3f5b50238f07e44481fcc2c1bdd1596a5a5be95987838b060` |
| pulse7.exe | RC 0.3.3 | `2190a30ded7eaa93d4...`（含路径修复 + 沙盒阻止消息） |
| MinGit 2.46.2 | 冻结 | `0dca60869825ceb8...` |
| Sandboxie Classic 5.73.2 | 冻结 | `18239310d6ad247e...` |
| ripgrep 13.0.0 | 冻结 | `ab5595a4f7a6b918...` |

**Git 标签链**：`m2-freeze` → `m3-complete` → `rc-0.1` → `rc-0.2` → `rc-0.3` → `rc-0.3.3`
...
3. **2-3 个 IT 同事真实试用** — 用 RC 0.3.3 跑 3-5 个真实任务；**下一轮开发内容由反馈决定**
