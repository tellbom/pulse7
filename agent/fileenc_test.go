...
func mustReadBeforeMutation(t *testing.T, r *Registry, path string) {
	t.Helper()
	if _, err := r.toolRead(mustJSON(t, map[string]interface{}{"path": path})); err != nil {
		t.Fatalf("read before mutation failed: %v", err)
	}
}
