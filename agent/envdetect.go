...
func selectSandboxMode(cfg *config, ws string, verbose bool) (sandboxRunner, string, []string) {
	...
	if err := ensureSandboxieConfig(startExe, cfg.box, ws, filepath.Join(cfg.exeDirStore(), "data", "tasks")); err != nil {
		return job(fmt.Sprintf("Sandboxie required direct-write paths unavailable: %v", err))
	}
...
func doctorCmd(cfg *config) {
	...
	fmt.Println("HTTP: this doctor process is not listening (address=127.0.0.1 port=none token=inactive); use pulse7 serve to start HTTP")
	ws, _ := filepath.Abs(cfg.workspace)
	fmt.Printf("exe-dir: %s\n", cfg.exeDirStore())
	probe := filepath.Join(cfg.exeDirStore(), "data")
	if err := os.MkdirAll(probe, 0o755); err != nil {
		fmt.Printf("data writable: FAIL (%v)\n", err)
	} else {
		fmt.Printf("data writable: OK (%s)\n", probe)
	}
	...
	if err != nil {
		...
		fmt.Printf("global config: %s (exists=%v)\n", path, fileExists(path))
	}
	projectPath := projectConfigPath(ws)
	fmt.Printf("project config: %s (exists=%v)\n", projectPath, fileExists(projectPath))
