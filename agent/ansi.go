...
func newConsoleWriter() *consoleWriter {
	return newConsoleWriterFor(^uintptr(10), os.Stdout) // STD_OUTPUT_HANDLE = -11
}

func newConsoleErrorWriter() *consoleWriter {
	return newConsoleWriterFor(^uintptr(11), os.Stderr) // STD_ERROR_HANDLE = -12
}

func newConsoleWriterFor(handle uintptr, fallback *os.File) *consoleWriter {
	...
	return &consoleWriter{h: h, isConsole: ok != 0, fallback: fallback}
}

func (w *consoleWriter) Write(p []byte) (int, error) {
	if !w.isConsole {
		return w.fallback.Write(p) // redirected: raw UTF-8 bytes
	}
	if len(p) == 0 {
		return 0, nil
	}
