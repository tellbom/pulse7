...
func (r *Registry) toolTree(argsJSON string) (string, error) {
	...
	if err := r.buildTreeLines(root, root, 0, a.MaxDepth, a.MaxEntries, &lines, &total, &omittedDirs, &omittedFiles, ""); err != nil {
		return "", err
	}
	if len(lines) == 0 {
...
func (r *Registry) buildTreeLines(root, dir string, depth, maxDepth, maxEntries int,
	lines *[]string, total, omitDirs, omitFiles *int, prefix string) error {
	...
	for _, e := range entries {
		name := e.Name()
		if strings.EqualFold(name, ".git") || (e.IsDir() && (treeSkipDirs[name] || strings.HasPrefix(name, "."))) {
			continue
		}
		if _, err := r.absPath(filepath.Join(dir, name)); err != nil {
			return err
		}
		if e.IsDir() {
			dirs = append(dirs, treeEntry{name: name, isDir: true})
		} else {
			files = append(files, treeEntry{name: name})
		}
	}
	...
	for _, d := range dirs {
		...
		safeSub, err := r.absPath(sub)
		if err != nil {
			return err
		}
		n, err := countDirectChildren(safeSub)
		if err != nil {
			return err
		}
		conn := "├── "
		...
		*total++
		count++
		if err := r.buildTreeLines(root, safeSub, depth+1, maxDepth, maxEntries, lines, total, omitDirs, omitFiles, prefix+"│   "); err != nil {
			return err
		}
	}
	for _, f := range files {
...
func countDirectChildren(dir string) (int, error) {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return 0, err
	}
