...
func ensureSandboxieConfig(startExe, box string, openPaths ...string) error {
	...
	wantedOpen := make([]string, 0, len(openPaths))
	for _, path := range openPaths {
		wantedOpen = append(wantedOpen, path+`\*`)
	}
	inBox := false
	hasBox := false
	hasEnabled := false
	...
	for _, l := range lines {
		...
		if strings.HasPrefix(lower, "openfilepath=") {
			configured := strings.TrimSpace(t[len("OpenFilePath="):])
			for _, wanted := range wantedOpen {
				if strings.EqualFold(configured, wanted) {
					hasOpen[wanted] = true
				}
			}
		}
	...
	for _, wanted := range wantedOpen {
		if !hasOpen[wanted] {
			if out, err := runSbieIni(sbieIni, "append", box, "OpenFilePath", wanted); err != nil {
