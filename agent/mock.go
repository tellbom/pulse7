...
func runMock(seconds int) {
	...
	mux.HandleFunc("/v1/chat/completions", func(w http.ResponseWriter, r *http.Request) {
		...
		hasB4bg, hasB4cross, hasB4detached := false, false, false
		hasB420, hasB4ExitNormal, hasB4ExitError, hasB4ExitCtrlC := false, false, false, false
		hasB4Timeout, hasB4Warn := false, false
		for _, m := range req.Messages {
			...
			if m.Role == openai.ChatMessageRoleUser {
				...
				if strings.Contains(m.Content, "B4-BACKGROUND") {
					hasB4bg = true
				}
				if strings.Contains(m.Content, "B4-CROSS") {
					hasB4cross = true
				}
				if strings.Contains(m.Content, "B4-DETACHED") {
					hasB4detached = true
				}
				if strings.Contains(m.Content, "B4-20") {
					hasB420 = true
				}
				if strings.Contains(m.Content, "B4-EXIT-NORMAL") {
					hasB4ExitNormal = true
				}
				if strings.Contains(m.Content, "B4-EXIT-ERROR") {
					hasB4ExitError = true
				}
				if strings.Contains(m.Content, "B4-EXIT-CTRLC") {
					hasB4ExitCtrlC = true
				}
				if strings.Contains(m.Content, "B4-TIMEOUT") {
					hasB4Timeout = true
				}
				if strings.Contains(m.Content, "B4-WARN") {
					hasB4Warn = true
				}
			}
		}
		...
		if hasB4ExitError && nTools > 0 {
			http.Error(w, "B4 injected execution error after background start", 500)
			return
		}
		if hasB4ExitCtrlC && nTools > 0 {
		...
		// Shadow the package helper for this request so every existing tool-call
		// branch records that it already emitted finish_reason=tool_calls.
		emitCalls := func(w http.ResponseWriter, fl http.Flusher, calls []mockCall) {
			emitCalls(w, fl, calls)
			terminalWritten = true
		}

		switch {
		case len(req.Tools) > 0 && hasB420:
			if nTools >= 40 {
				sseContent(w, fl, "MOCK-FINAL: 20 background start/kill cycles complete.")
			} else if nTools%2 == 0 {
				i := nTools/2 + 1
				emitCalls(w, fl, []mockCall{{0, fmt.Sprintf("b4-20-start-%d", i), "shell",
					fmt.Sprintf(`{"command":"echo CYCLE-%d & ping -n 30 127.0.0.1 >nul","background":true}`, i)}})
			} else {
				emitCalls(w, fl, []mockCall{{0, fmt.Sprintf("b4-20-kill-%d", nTools/2+1), "task_kill",
					fmt.Sprintf(`{"task_id":%q}`, mockTaskID(req.Messages))}})
			}
		case len(req.Tools) > 0 && (hasB4ExitNormal || hasB4ExitError || hasB4ExitCtrlC):
			if nTools == 0 {
				name := "b4-exit-normal-leak.txt"
				if hasB4ExitError {
					name = "b4-exit-error-leak.txt"
				} else if hasB4ExitCtrlC {
					name = "b4-exit-ctrlc-leak.txt"
				}
				emitCalls(w, fl, []mockCall{{0, "b4-exit-start", "shell", fmt.Sprintf(
					`{"command":"ping -n 60 127.0.0.1 >nul & echo LEAKED>%s","background":true}`, name)}})
			} else {
				sseContent(w, fl, "MOCK-FINAL: normal exit cleanup trigger complete.")
			}
		case len(req.Tools) > 0 && hasB4Timeout:
			if nTools == 0 {
				emitCalls(w, fl, []mockCall{{0, "b4-timeout", "shell",
					`{"command":"start \"\" /b cmd.exe /d /c \"ping -n 8 127.0.0.1 >nul & echo LEAKED>b4-timeout-leak.txt\" & ping -n 30 127.0.0.1 >nul"}`}})
			} else {
				sseContent(w, fl, "MOCK-FINAL: foreground timeout trigger complete.")
			}
		case len(req.Tools) > 0 && hasB4Warn:
			taskID := mockTaskID(req.Messages)
			switch nTools {
			case 0:
				emitCalls(w, fl, []mockCall{{0, "b4-warn-start", "shell", `{"command":"echo WARN-READY & ping -n 30 127.0.0.1 >nul","background":true}`}})
			case 1:
				emitCalls(w, fl, []mockCall{{0, "b4-warn-wait", "shell", `{"command":"ping -n 3 127.0.0.1 >nul & echo WARN-WAITED"}`}})
			case 2:
				emitCalls(w, fl, []mockCall{{0, "b4-warn-kill", "task_kill", fmt.Sprintf(`{"task_id":%q}`, taskID)}})
			default:
				sseContent(w, fl, "MOCK-FINAL: warning thresholds did not block execution.")
			}
		case len(req.Tools) > 0 && hasB4bg:
			taskID := mockTaskID(req.Messages)
			switch nTools {
			case 0:
				emitCalls(w, fl, []mockCall{{0, "b4-bg-start", "shell", `{"command":"for /l %%i in (1,1,30) do @(echo BG-%%i & ping -n 2 127.0.0.1 >nul)","background":true}`}})
			case 1:
				emitCalls(w, fl, []mockCall{{0, "b4-bg-output", "task_output", fmt.Sprintf(`{"task_id":%q,"offset":0,"limit":4096}`, taskID)}})
			case 2:
				emitCalls(w, fl, []mockCall{{0, "b4-bg-kill", "task_kill", fmt.Sprintf(`{"task_id":%q}`, taskID)}})
			default:
				sseContent(w, fl, "MOCK-FINAL: background start/output/kill complete.")
			}
		case len(req.Tools) > 0 && hasB4cross:
			switch nTools {
			case 0:
				emitCalls(w, fl, []mockCall{{0, "b4-cross-start", "shell", `{"command":"start \"\" /b cmd.exe /d /c \"ping -n 2 127.0.0.1 >nul & echo CROSS-READY>cross-ready.txt\""}`}})
			case 1:
				emitCalls(w, fl, []mockCall{{0, "b4-cross-use", "shell", `{"command":"ping -n 3 127.0.0.1 >nul & type cross-ready.txt"}`}})
			default:
				if strings.Contains(lastContent, "exitcode=0") && strings.Contains(lastContent, "CROSS-READY") {
					sseContent(w, fl, "MOCK-FINAL: cross-command process survived and was used.")
				} else {
					sseContent(w, fl, "MOCK-FAIL: cross-command process was not usable; inspect shell results.")
				}
			}
		case len(req.Tools) > 0 && hasB4detached:
			if nTools == 0 {
				emitCalls(w, fl, []mockCall{{0, "b4-detached", "shell", `{"command":"ping -n 120 127.0.0.1 >nul","background":true,"detached":true}`}})
			} else {
				sseContent(w, fl, "MOCK-FINAL: detached process started.")
			}
		...
		finishMockStream(w, fl, terminalWritten)
	})
...
func mockTaskID(messages []openai.ChatCompletionMessage) string {
	for i := len(messages) - 1; i >= 0; i-- {
		if messages[i].Role != openai.ChatMessageRoleTool {
			continue
		}
		for _, field := range strings.Fields(messages[i].Content) {
...
func finishMockStream(w http.ResponseWriter, fl http.Flusher, terminalWritten bool) {
	if !terminalWritten {
		writeSSE(w, fl, map[string]interface{}{}, "stop")
	}
