...
func (r *Registry) toolGrep(argsJSON string) (string, error) {
	...
	if err := r.validateSearchTree(root); err != nil {
		return "", err
	}
	// try ripgrep first
...
func (r *Registry) validateSearchTree(root string) error {
	return r.validateWorkspaceTree(root, true)
}

func (r *Registry) validateWorkspaceTree(root string, skipNoise bool) error {
	return filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if path != root && strings.EqualFold(d.Name(), ".git") {
			if d.IsDir() {
				return filepath.SkipDir
			}
			return nil
		}
		if skipNoise && path != root && d.IsDir() && (treeSkipDirs[d.Name()] || strings.HasPrefix(d.Name(), ".")) {
			return filepath.SkipDir
		}
		resolved, err := r.absPath(path)
		if err != nil {
			return err
		}
		if !skipNoise {
			rootPath, _, _, err := resolveExistingPrefix(r.workspace)
			if err != nil {
				return err
			}
			if err := requirePathWithin(rootPath, resolved); err != nil {
				return fmt.Errorf("path %s resolves outside workspace %s (resolved=%s)", path, rootPath, resolved)
			}
		}
		return nil
	})
