...
- 写读不一致和单文件拖垮列表已修复；前端需要呈现 sessions.errors，不能静默省略坏文件。
- JSONL仍有1MiB记录限制，大模型单条内容超过限制会明确失败；大内容外置/分块未实现。未修改已隔离原文件。
- 配置值域不等于资源保证：大context、后台输出和Job内存上限在低配Win7仍可能造成内存/磁盘压力；未做这些上限的压力验收。
- 最近工作区跨serve重启记忆：仅待办，未修改契约。
- GET config是当前运行值；全局保存仍受重启时CLI/项目优先级影响；前端区分saved和restartRequiredFields。
- 外部并行加入的fsDirs草稿未接入路由，不属于本轮已验收功能。前端资源与该草稿保持在工作树，不混入单独提交。