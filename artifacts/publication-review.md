...
状态：用户批准清洁发布方案后已完成推送，最终结果见文末。下方“推送阻断”保留执行前的检查事实，不代表仍在等待批准。
...
- 整理提交为`b889655a24337b8e393dff22d7df5accc74b1658`。其完整文件树`12908284713034a8cca8c4f18bbd382a028a4892`被原样用于创建无父提交的发布快照`eb7a322ad6e0f94aa00e70c04ebae48155328e0f`，作为新main；未merge/rebase任何旧main代码。
- 本地归档分支`archive/main-before-publication-20260912`保留旧main，`archive/development-before-publication-20260912`保留整理后的开发历史；pre-phase3仍保留。上述归档和本地旧tag未推送。
- 重新读取远端引用并与检查时OID逐项比较；使用`push --atomic`和每个引用的显式`--force-with-lease=<ref>:<expected-oid>`，关闭followTags，一次替换main并删除已确认含凭据的6个其他分支和9个tag，全部成功。
- fetch后本地main与origin/main均为上述清洁快照，工作树clean。agent/ui目录树与整理前完全相同。GitHub仓库API返回200、default_branch=main；GitHub raw README返回200且与本地内容一致。
- 远端保留main和m2-freeze/m3-complete/rc-0.1三个旧tag；对这些引用可达的399个文本对象（含提交消息与UTF-16文本）复扫，未命中sk格式密钥或私钥标记。11个二进制对象未纳入该文本扫描；不将有限扫描写成对所有历史二进制或平台缓存的绝对保证。新main不包含这些旧tag的祖先链。
- 本节是推送后追加的文档记录，不改变产品源码。随后仅将本记录以正常后继提交同步main。没有打新的功能验收tag，没有改变原验收报告中的延期/未验证结论。

旧敏感凭据已经通过远端历史暴露，作废/轮换仍由凭据持有人执行；本次Git操作不声称完成服务端撤销凭据、平台缓存清除或第三方克隆清理。
